...
function ResponsableMagasinPage() {
  ...
  // La monture qui vient d'entrer en stock : affichée en popup le temps que le magasinier
  // lise l'emplacement où la ranger, puis refermée pour laisser place au scan suivant.
  const [montureRecue, setMontureRecue] = useState<SendBoxItem | null>(null)
  ...
  async function recevoirMonture(barcode: string) {
    ...
    try {
      ...
      let recue: SendBoxItem | undefined
      setCartonItems(items => items.map(item => {
        if (String(item.barcode || '').toLowerCase() !== cible && String(item.reference || '').toLowerCase() !== cible) return item
        recue = { ...item, received: true, stock_location_code: emplacement || item.stock_location_code }
        return recue
      }))
      ...
      setCartonTone('green')
      // Popup avec le code-barres et l'emplacement du stock magasin, comme sur l'écran de
      // scan : le magasinier vient de recevoir la monture en main, elle lui redonne
      // l'emplacement exact sans qu'il ait à rechercher la ligne dans le tableau.
      setMontureRecue(recue || { id: 0, barcode: propre, stock_location_code: emplacement, received: true })
      setScanMonture('')
  ...
  return (
    <div className={dark ? 'dark' : ''}>
      <div className="flex min-h-screen bg-slate-50 dark:bg-slate-900">
                      ...
                      <div className={CARD}>
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <p className="text-sm font-bold text-slate-900 dark:text-white truncate">Bloc {blocCourant.cle}</p>
                          </div>
        ...
        {/* ── Popup de réception d'une monture (cartons reçus) ───────────────── */}
        {montureRecue && (
          <MontureRecueModal
            item={montureRecue}
            onClose={() => {
              setMontureRecue(null)
              // La douchette doit pouvoir enchaîner sur la monture suivante sans clic.
              montureRef.current?.focus()
            }}
          />
        )}
        ...
        <div className="p-4 space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-900">
              {glass.photo_monture_url ? (
                <img src={glass.photo_monture_url} alt={glassRef(glass)} className="h-40 w-full object-cover" />
              ) : (
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-900">
              {glass.photo_branche_url ? (
                <img src={glass.photo_branche_url} alt="Branche" className="h-40 w-full object-cover" />
              ) : (
...
// ── Étiquette de la monture reçue (code-barres + emplacement magasin) ──────────
// Même gabarit 56 mm que l'écran Scan côté réception (scan.tsx) et que la popup
// équivalente côté vendeuse (vendeuse.tsx). Dupliqué plutôt que partagé : chaque
// poste de ce projet vit dans son propre bundle (vite.config.ts).
const LABEL_CSS = `
  @page { margin: 0; }
...
async function drawPrintableBarcode(target: SVGSVGElement, value: string, showValue = true) {
  if (!value) return
 ...
 *  du type « RAYON-A-ETA-01-BAC-A-POS-01 » se replie correctement. */
...
async function captureBarcodeSvg(value: string) {
  ...
  try {
    await drawPrintableBarcode(svg, value, false)
...
/** Une copie image part sur le disque : le magasinier doit pouvoir retrouver
 *  l'étiquette d'une monture reçue sans avoir à la réimprimer. */
...
// ── Popup de réception d'une monture ────────────────────────────────────────────
/** Reprend la mise en page de l'étape 3 de scan.tsx (code-barres + emplacement) : c'est
 *  l'écran que le magasinier connaît déjà pour « où poser la monture qu'il a en main ». */
function MontureRecueModal({ item, onClose }: { item: SendBoxItem; onClose: () => void }) {
  const barcode = item.barcode || item.reference || ''
  const [busy, setBusy] = useState<'print' | 'download' | ''>('')

  const label: PrintableLabel = {
    title: item.brand || '—',
    reference: item.reference || barcode,
    barcodeValue: barcode,
    metaLeft: item.stock_location_code || 'Emplacement non attribué',
    metaRight: fmtFCFA(item.price),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-100 dark:border-slate-700">
          <Pastille color={C.success}>{ic.check()}</Pastille>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{item.reference || barcode || '—'}</p>
            <p className="text-xs text-slate-400 truncate">{item.brand || 'Monture reçue en stock'}</p>
          </div>
          <button
            onClick={onClose}
            className="flex-shrink-0 p-1.5 text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-all"
        ...
        <div className="p-4 space-y-4">
          <div className="flex flex-col items-center gap-2 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700">
            <div className="flex h-16 w-full items-center justify-center">
              {barcode
                ? <Code128 value={barcode} height={48} showValue={false} />
                : <span className="text-xs text-slate-400">Aucun code-barres</span>}
            </div>
            <p className="font-mono text-sm font-bold tabular-nums text-slate-900 dark:text-white">{barcode || '—'}</p>
          </div>

          <div className="rounded-xl p-4 text-center" style={{ background: `${C.success}18` }}>
            <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: C.success }}>
              À ranger en
            </p>
            <p className="mt-1 text-2xl font-black tabular-nums" style={{ color: C.success }}>
        ...
        <div className="space-y-2 border-t border-slate-100 px-4 py-3 dark:border-slate-700">
          ...
          <button
            onClick={onClose}
            className="w-full rounded-xl px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"
          >
            OK, rangée
          </button>
        </div>
